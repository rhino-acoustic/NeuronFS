...
def find_gemini():
    """Gemini CLI 경로 동적 탐색 (SSOT: multi_agent.go 로직 이식)"""
    ...
    candidates = [
        ...
        os.path.join(userprofile, ".local", "bin", "gemini"),
        "gemini"
    ]
    for c in candidates:
        if shutil.which(c) or os.path.exists(c):
            return c
    return "gemini" # Fallback to path
...
def main():
    print("NeuronFS Identity Tag A/B Benchmark (v2 - Dynamic Path)")
