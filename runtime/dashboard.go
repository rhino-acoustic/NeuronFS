...
// ?Ä?Ä?Ä Health check models ?Ä?Ä?Ä
...
// ?Ä?Ä?Ä JSON models for API ?Ä?Ä?Ä
...
// ?Ä?Ä?Ä Check if a process with given image name is running ?Ä?Ä?Ä
...
// ?Ä?Ä?Ä Check if a node.js script is running ?Ä?Ä?Ä
...
// ?Ä?Ä?Ä Count neuron files ?Ä?Ä?Ä
...
// ?Ä?Ä?Ä Build health JSON ?Ä?Ä?Ä
func buildHealthJSON(brainRoot string) HealthJSON {
	processes := []ProcessHealth{
		// ?∏ÌîÑ???∞Î™¨
		{Name: "neuronfs", Role: "?∏Ï? ?îÏßÑ (API ?úÎ≤Ñ + ?Ä?úÎ≥¥??", Running: isProcessRunning("neuronfs.exe")},
		{Name: "agent-bridge", Role: "CDP ?êÏù¥?ÑÌä∏ ?µÏã† Î∏åÎ¶øÏßÄ", Running: isNodeScriptRunning("agent-bridge")},
		{Name: "auto-accept", Role: "CDP ?êÎèô ?òÎùΩ + ÍµêÏ†ï Í∞êÏ?", Running: isNodeScriptRunning("auto-accept")},
		{Name: "watchdog", Role: "?ÑÏ≤¥ ?ÑÎ°ú?∏Ïä§ ?ùÏ°¥ Í∞êÏãú + harness Ï£ºÍ∏∞ ?§Ìñâ", Running: isNodeScriptRunning("watchdog") || isProcessRunning("powershell.exe")},
	}

	return HealthJSON{
		API:        true, // ???ëÎãµ?????úÏ†ê??API???¥ÏïÑ?àÏùå
		Processes:  processes,
		OS:         runtime.GOOS,
		BrainRoot:  brainRoot,
...
// ?Ä?Ä?Ä Build brain JSON from scan ?Ä?Ä?Ä
...
// ?Ä?Ä?Ä Dashboard Server (--dashboard mode) ?Ä?Ä?Ä
...
func startDashboard(brainRoot string, port int) {
	...
	mux.HandleFunc("/api/report", withCORSDashboard(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{
			...
			"pending":  pending,
			"ack":      "?àÎ°ú??Î≥¥Í≥†Í∞Ä ?ïÏù∏?òÏóà?µÎãà?? ?¨Ïö©?êÏùò ?îÏ≤≠ Ï≤òÎ¶¨ ???îÎ°ú?ÖÌï©?àÎã§.",
		})
	}))
