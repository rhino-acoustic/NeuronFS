...
// supervisor.go ??NeuronFS ?§Ïù¥?∞Î∏å ?ÑÎ°ú?∏Ïä§ Îß§Îãà?Ä
//
// watchdog.ps1 + ?ÑÎ°ú?∏Ïä§ Í¥ÄÎ¶¨Î? Go Î∞îÏù¥?àÎ¶¨Î°??µÌï©.
//
//
...
func runSupervisor(brainRoot string) {
	...
	fmt.Println("")
	fmt.Println("?î‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïó")
	fmt.Println("?? NeuronFS Supervisor v2.1 ??Self-Monitoring      ??)
	fmt.Println("?? ?ÑÎ°ú?∏Ïä§ ?êÎèô?¨Ïãú??+ ?êÍ∏∞ Í∞êÏãú                  ??)
	fmt.Println("?ö‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïù")
	fmt.Println("")
	...
	svLog("?Å‚îÅ??Í∞êÏãú Î£®ÌîÑ ÏßÑÏûÖ ?Å‚îÅ??)
	for {
		select {
		case <-sigCh:
		...
		case <-lockTk.C:
			for _, c := range children {
				...
				if locked && wasRunning {
					svLog(fmt.Sprintf("?îí %s ??PM lock. Ï§ëÏ?.", c.Name))
					c.stop()
				}
			}
		case <-decayTk.C:
...
func svSupervise(c *ChildSpec, stopCh <-chan struct{}) {
	...
	for {
		...
		svLog(fmt.Sprintf("??%s ?úÏûë (#%d)", c.Name, c.restartCount))
		...
		if c.isLocked() {
			svLog(fmt.Sprintf("?îí %s Ï¢ÖÎ£å ??lock ?ÄÍ∏?, c.Name))
			continue
		}
...
func svConsolidationCheck(brainRoot string, nfsExe string) {
	...
	if count > 50 {
		...
		// ?Ä?Ä [??git ?? ?ïÏ∂ï ???∞Ïù¥???àÏ†ÑÎß? ?Ä?Ä
		svLog("\033[90m[GIT] Pre-consolidation snapshot triggered (??git ÏßÑÌñâ)...\033[0m")
		...
		if err := cmd.Start(); err != nil {
			...
			go func() {
				...
				// ?Ä?Ä [NAS ?ëÏßà ?∞Ïù¥???Ä?ÑÏä§?¨ÌîÑ ?ÑÏπ¥?¥Îπô ?¥Ïãù] ?Ä?Ä
