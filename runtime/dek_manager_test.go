...
func TestEncryptDecryptWithKeyStore(t *testing.T) {
	...
	plaintext := []byte("cortex/frontend/hooks_pattern 뉴런 데이터")

	// 암호화
	ciphertext, nonce, err := EncryptNeuron(plaintext, dek)
	if err != nil {
	...
	// 이전 키로 복호화 (retired key 조회)
	oldKey, _, _ := ks.GetKeyByID(ks.active.ID)
	// 실제로는 이전 키 ID를 ciphertext 메타데이터에 저장해야 함
	// 여기서는 원래 dek를 직접 사용
...
func TestKeyIntegrity(t *testing.T) {
	...
	// 키 파일 변조
	keyPath := filepath.Join(dir, ".neuronfs", "keys", meta.ID, "key.json")
