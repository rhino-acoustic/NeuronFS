...
// ?Å‚îÅ??activationBar ?Å‚îÅ??func TestActivationBar_AllTiers(t *testing.T) {
	tests := []struct {
		input    int
		expected string
	}{
...
// ?Å‚îÅ??countNeuronFiles ?Å‚îÅ??func TestCountNeuronFiles(t *testing.T) {
	dir := t.TempDir()
...
// ?Å‚îÅ??signalNeuron ??bomb signal ?Å‚îÅ??func TestSignalNeuron_Bomb(t *testing.T) {
...
// ?Å‚îÅ??signalNeuron ??memory signal ?Å‚îÅ??func TestSignalNeuron_Memory(t *testing.T) {
	dir := setupTestBrain(t)
...
// ?Å‚îÅ??signalNeuron ??unknown type ?Å‚îÅ??func TestSignalNeuron_UnknownType(t *testing.T) {
	dir := setupTestBrain(t)
...
// ?Å‚îÅ??signalNeuron ??nonexistent neuron ?Å‚îÅ??func TestSignalNeuron_NotFound(t *testing.T) {
	dir := setupTestBrain(t)
...
// ?Å‚îÅ??emitRules ?Å‚îÅ??func TestEmitRules_Format(t *testing.T) {
	dir := setupTestBrain(t)
	brain := scanBrain(dir)
...
// ?Å‚îÅ??runStats ?Å‚îÅ??func TestRunStats_NoCrash(t *testing.T) {
	dir := setupTestBrain(t)
...
// ?Å‚îÅ??runVacuum ?Å‚îÅ??func TestRunVacuum_NoCrash(t *testing.T) {
	dir := setupTestBrain(t)
...
// ?Å‚îÅ??runDecay ?Å‚îÅ??func TestRunDecay_NoCrash(t *testing.T) {
	dir := setupTestBrain(t)
...
// ?Å‚îÅ??printDiag ?Å‚îÅ??func TestPrintDiag_NoCrash(t *testing.T) {
	dir := setupTestBrain(t)
	brain := scanBrain(dir)
	result := runSubsumption(brain)
...
// ?Å‚îÅ??printDiag with bomb ?Å‚îÅ??func TestPrintDiag_WithBomb(t *testing.T) {
	dir := setupTestBrain(t)
...
// ?Å‚îÅ??deduplicateNeurons ?Å‚îÅ??func TestDeduplicateNeurons_NoCrash(t *testing.T) {
	dir := setupTestBrain(t)
	deduplicateNeurons(dir)
	t.Logf("OK: deduplicateNeurons completed without crash")
}

// ?Å‚îÅ??buildHealthJSON ?Å‚îÅ??func TestBuildHealthJSON(t *testing.T) {
	dir := setupTestBrain(t)
