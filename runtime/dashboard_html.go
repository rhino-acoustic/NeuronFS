...
<title>NeuronFS v5 ???∏Ï? ?îÏßÑ</title>
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ?Ä?Ä Detail Panel (appears on sphere click) ?Ä?Ä */
  .detail-panel {
    padding: 20px 24px; border-bottom: 1px solid #1a1a2e;
...
<div class="app">
  ...
  <div class="sidebar">
    ...
    <div class="stats">
      <div class="stat"><div class="stat-value" id="s-neurons">0</div><div class="stat-label">?¥Îü∞</div></div>
      <div class="stat"><div class="stat-value" id="s-activation">0</div><div class="stat-label">?úÏÑ±??/div></div>
      <div class="stat"><div class="stat-value" id="s-regions">0</div><div class="stat-label">?ÅÏó≠</div></div>
    </div>

    <div class="search-bar">
      <input type="text" id="searchInput" placeholder="?¥Îü∞ Í≤Ä??(Ctrl+K)" oninput="filterNeurons()">
    </div>
    ...
    <div class="recent-neurons-section" id="recentSection" style="padding: 16px 24px; border-bottom: 1px solid #1a1a2e;">
      <div class="add-label" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
        <span>?å± ÏµúÍ∑º ?êÎùº???¥Îü∞ (Live)</span>
      </div>
    ...
    <div class="add-section" id="addSection">
      <div class="add-label">???¥Îü∞ ?ùÏÑ±</div>
      <div class="add-row">
        <select id="addRegion"></select>
        <input type="text" id="addPath" placeholder="Í≤ΩÎ°ú (?? methodology/tdd)">
        <button onclick="addNeuron()">+</button>
      </div>
    </div>

    <div class="sandbox-section" id="sandboxSection">
      <div class="add-label">?ß™ Sandbox ??Í∑úÏπô ?§Ìóò</div>
      <textarea id="sandboxText" placeholder="??Ï§ÑÏóê ?òÎÇò??Í∑úÏπô ?ÖÎ†•&#10;?? Á¶ÅÏù∏?ºÏù∏?§Ì???#10;    ??ÉÅ_?Ä?ÖÏ≤¥??></textarea>
      <div class="sandbox-row">
        <button class="btn btn-primary" onclick="applySandbox()" style="font-size:10px">???ÅÏö©</button>
        <button class="btn btn-ghost" onclick="clearSandbox()" style="font-size:10px">?óë Ï¥àÍ∏∞??/button>
      </div>
    ...
    <div class="evo-section" id="evoSection">
      <div class="add-label" style="margin-bottom:12px;display:flex;align-items:center;gap:6px;justify-content:space-between">
        <span>?ß¨ EVOLUTION TIMELINE</span>
    ...
    <div class="controls">
      ...
      <button class="btn btn-add" onclick="toggleAdd()">+ ?¥Îü∞</button>
      <button class="btn btn-ghost" onclick="doDedup()">?? DEDUP</button>
      <button class="btn btn-ghost" onclick="toggleEvo()" style="font-size:10px">?ß¨</button>
      <button class="btn btn-ghost" onclick="toggleSandbox()" style="font-size:10px">?ß™</button>
      <select id="bombRegion" style="background:#1a1a2e;color:#fca5a5;border:1px solid #7f1d1d;border-radius:6px;padding:6px 8px;font-size:10px;cursor:pointer;"><option value="">?? ?ÅÏó≠ ?†ÌÉù</option></select>
...
const regionEmoji = {
  brainstem: '??', limbic: '?íì', hippocampus: '?ìù',
  sensors: '?ëÅÔ∏?, cortex: '?ß†', ego: '?é≠', prefrontal: '?éØ'
};

// ?Ä?Ä Three.js Setup ?Ä?Ä
function initThree() {
...
function onMouseMove(e) {
  ...
  if (hits.length > 0) {
    ...
    if (hitObj.userData && hitObj.userData.isNeuron) {
      ...
      tooltip.innerHTML = '<span class="tt-region">?ß† ' + hitObj.userData.region + '/' + dispPath.split('/').pop() + '</span>' +
        '<span class="tt-stats">?úÏÑ±??Synapse): ' + hitObj.userData.counter + '</span>';
    ...
    if (entry) {
      tooltip.innerHTML = '<span class="tt-region">' + (regionEmoji[entry.region.name]||'') + ' ' + entry.region.name + '</span>' +
        '<span class="tt-stats">?¥Îü∞ ' + entry.neuronCount + ' | ?úÏÑ±??' + entry.totalAct + '</span>';
...
// ?Ä?Ä Select Region (show detail) ?Ä?Ä
function selectRegion(name) {
  ...
  document.getElementById('detail-icon').textContent = regionEmoji[name] || '?ìÅ';
  document.getElementById('detail-name').textContent = name.toUpperCase() + ' ??' + (region.ko || '');

  // Axons
  let axonHtml = '';
  if (region.axons && region.axons.length > 0) {
    axonHtml += '<div style="font-size:10px;color:#666;margin-bottom:4px;">Ï∂ïÏÇ≠ ?∞Í≤∞:</div>';
  ...
  if (region.neurons) {
    ...
    sorted.forEach(n => {
      ...
      let signals = '';
      if (n.dopamine > 0) signals += '?ü¢';
      if (n.hasBomb) signals += '??';
      if (n.isDormant) signals += '?í§';
      if (pol > 0.6) signals += '??;
      else if (pol < 0.3 && intensity > 3) signals += '??;

      let strengthHtml = '';
      if (intensity >= 10) strengthHtml = '<span class="n-strength n-str-abs">?àÎ?</span>';
      else if (intensity >= 5) strengthHtml = '<span class="n-strength n-str-must">Î∞òÎìú??/span>';
...
function updateUI(data) {
  ...
  document.getElementById('recentGrid').innerHTML = recentHtml || '<div style="color:#666; font-size:9px; grid-column:span 2; text-align:center;">?ùÏû• Ï§ëÏù∏ ?¥Îü∞ Í∏∞Î°ù ?ÜÏùå</div>';
...
async function loadBrain() {
  try {
    ...
    if (previousNeuronCount > 0 && brainData.totalNeurons > previousNeuronCount) {
        ...
        showToast('?å± ?êÍ? ÏßÑÌôî Î∞úÏÉù: +' + diff + ' ?†Í∑ú Í∑úÏπô ?°Ïàò?ÑÎ£å');
    }
    previousNeuronCount = brainData.totalNeurons;
  } catch(e) { /* silent ??Á¶Åconsole_log */ }
}
...
function updateBombDropdown() {
  ...
  sel.innerHTML = '<option value="">?? ?ÅÏó≠ ?†ÌÉù</option>';
...
async function doBomb() {
  ...
  if (!region) { showToast('?†Ô∏è ?ÅÏó≠??Î®ºÏ? ?†ÌÉù?òÏÑ∏??); return; }
...
async function addNeuron() {
  ...
  showToast('?å± ?¥Îü∞ ?ùÏÑ±: ' + region + '/' + path);
  loadBrain();
}
...
function filterNeurons() {
  ...
  let nHtml = '<div style="font-size:10px;color:#666;margin-bottom:6px;">Í≤Ä??Í≤∞Í≥º: ' + results.length + 'Í±?/div>';
  results.forEach(n => {
    ...
    let strengthHtml = '';
    if (n.counter >= 10) strengthHtml = '<span class="n-strength n-str-abs">?àÎ?</span>';
    else if (n.counter >= 5) strengthHtml = '<span class="n-strength n-str-must">Î∞òÎìú??/span>';
    let signals = '';
    if (n.dopamine > 0) signals += '?ü¢';
    if (n.hasBomb) signals += '??';
    nHtml += '<div class="neuron-item">' +
  ...
  const panel = document.getElementById('detail');
  panel.classList.add('active');
  document.getElementById('detail-icon').textContent = '?îç';
  document.getElementById('detail-name').textContent = 'Í≤Ä?? "' + q + '"';
...
async function applySandbox() {
  ...
  if (!text) { showToast('?†Ô∏è Í∑úÏπô???ÖÎ†•?òÏÑ∏??); return; }
  ...
  if (data.paths && data.paths.length > 0) {
    pathsDiv.innerHTML = data.paths.map(p => '<div>??' + p + ' ?ùÏÑ±??/div>').join('');
  } else {
    pathsDiv.innerHTML = '<div>??' + (data.created || 0) + 'Í∞??ÅÏö©</div>';
  }
  showToast('?ß™ Sandbox: ' + (data.created || 0) + 'Í∞?Í∑úÏπô ?ÅÏö©');
  loadBrain();
}
...
function showToast(msg) {
  ...
  if (msg.includes('?†Ô∏è') || msg.includes('??')) t.classList.add('error'); else t.classList.remove('error');
...
async function loadEvolution() {
  try {
    ...
    if (events.length === 0) {
      container.innerHTML = '<div style="font-size:10px;color:#555;text-align:center;padding:20px">ÏßÑÌôî Í∏∞Î°ù ?ÜÏùå (git Ï¥àÍ∏∞???ÑÏöî)</div>';
      return;
    }

    const actionLabels = { created: '?¥Îü∞ ?ÑÏÉù', modified: '?úÎÉÖ??Í∞ïÌôî', suppressed: '?µÏ†ú Î∞úÎèô' };
    const actionIcons = { created: '?å±', modified: '??, suppressed: '?îÆ' };
    let html = '';
