...
var regionKo = map[string]string{
	"brainstem":   "?‘ì‹¬/ë³¸ëŠ¥",
	"limbic":      "ê°ì • ?„í„°",
	"hippocampus": "ê¸°ë¡/ê¸°ì–µ",
	"sensors":     "?˜ê²½ ?œì•½",
	"cortex":      "ì§€??ê¸°ìˆ ",
	"ego":         "?±í–¥/??,
	"prefrontal":  "ëª©í‘œ/ê³„íš",
}
...
func scanBrain(root string) Brain {
	...
	for name, regionPath := range regionsToScan {
		...
		// Scan flat neurons at region root (e.g., brainstem: ç¦fallback.1.neuron)
...
func injectToGemini(brainRoot string, rules string) {
	...
	// ê¸€ë¡œë²Œ ?¨ì¼ ê²½ë¡œ: USERPROFILE/.gemini/GEMINI.md
	home := os.Getenv("USERPROFILE")
	if home == "" {
...
func runWatch(brainRoot string) {
	...
	for {
		...
		case event, ok := <-watcher.Events:
			...
			// .git ?”ë ‰? ë¦¬ ?´ë²¤??ë¬´ì‹œ ??git snapshotê³??°ë“œ??ë°©ì?
			...
			if event.Op&(fsnotify.Create|fsnotify.Write) != 0 {
				...
				fmt.Printf("%s[%s] [PRUNE] ?°ë“œ ?œëƒ…???œê±°: %s%s\n", ansiDimGray, ts, relPath, ansiReset)
			}
...
func printDiag(brain Brain, result SubsumptionResult) {
	fmt.Println("?â•??NeuronFS v4.0 ??Folder-as-Neuron Engine ?â•??)
	fmt.Printf("  Brain: %s\n", brain.Root)
...
func tokenize(name string) []string {
	// ë°‘ì¤„ê³?ê³µë°± ëª¨ë‘ ë¶„ë¦¬?ë¡œ ì²˜ë¦¬
	normalized := strings.ReplaceAll(strings.ToLower(name), "_", " ")
	parts := strings.Fields(normalized) // Fields???°ì† ê³µë°±??ì²˜ë¦¬
	var tokens []string
	for _, p := range parts {
...
// jaccardSimilarity computes |A?©B| / |A?ªB| between two token sets
...
func rollbackNeuron(brainRoot string, neuronPath string) error {
	...
	if currentCounter <= 1 {
		fmt.Printf("[ROLLBACK] ? ï¸ %s counter already at minimum (%d)\n", neuronPath, currentCounter)
...
func signalNeuron(brainRoot string, neuronPath string, sigType string) error {
	...
	case "dopamine":
		...
		fmt.Printf("[SIGNAL] ?Ÿ¢ dopamine%d ??%s\n", nextDopa, neuronPath)
...
func logEpisode(brainRoot string, event string, detail string) {
	...
	if len(mems) >= maxEpisodes {
		...
		fmt.Printf("[MEMORY] ?—‘ï¸?Evicted %d old episodes (circular buffer %d)\n", evictCount, maxEpisodes)
	}
...
func autoReinject(brainRoot string) {
	...
	writeAllTiers(brainRoot)
	fmt.Printf("[INJECT] ?»ï¸  Mount set changed ??GEMINI.md updated\n")
}
...
func processInbox(brainRoot string) {
	...
	for _, line := range lines {
		...
		if err := json.Unmarshal([]byte(line), &entry); err != nil {
			fmt.Printf("[INBOX] ? ï¸ parse error: %s\n", line)
			continue
		}
		...
		if neuronPath == "" {
			...
			sanitized := strings.Map(func(r rune) rune {
				if (r >= 'a' && r <= 'z') || (r >= 'A' && r <= 'Z') || (r >= '0' && r <= '9') ||
					r == '_' || r == '-' ||
					(r >= 0xAC00 && r <= 0xD7AF) || // ?œê? ?Œì ˆ
					(r >= 0x3131 && r <= 0x318E) || // ?œê? ?ëª¨
					(r >= 0x4E00 && r <= 0x9FFF) { // ?œì CJK
					return r
				}
				return '_'
		...
		if strings.Contains(neuronPath, "..") || strings.Contains(neuronPath, `\`) ||
			...
			strings.Contains(neuronPath, "|") || strings.Contains(neuronPath, ">") {
			fmt.Printf("[SECURITY] ?›¡ï¸?Injection blocked: %s\n", neuronPath)
			continue
		}

		// ê¸°ê³„??ì¹?°¬(Dopamine Inflation) ?„í„°ë§?
		isPraise := false
		if entry.Type == "correction" && entry.Text == "PDì¹?°¬" {
			isPraise = true
		}
		...
		if isPraise {
			...
			if authorId != "pm" && authorId != "basement_admin" && !strings.Contains(authorId, "pd") {
				fmt.Printf("[INBOX] ?›¡ï¸??„íŒŒë¯??¸í”Œ?ˆì´??ì°¨ë‹¨ (ì¹¨í•´?? %s): %s\n", authorId, entry.Text)
				continue
			}
			// PM ì¹?°¬?€ ë°”ë¡œ ?„íŒŒë¯?ë°œí™”
			...
			fmt.Printf("[INBOX] ?Ÿ¢ PM ì¹?°¬ ?•ì¸ ???„íŒŒë¯?ë°°í¬: %s\n", neuronPath)
			processed++
			continue
		}
		...
		if info, err := os.Stat(fullPath); err == nil && info.IsDir() {
			...
			fmt.Printf("[INBOX] ?Œ± grow %s (counter=%d)\n", neuronPath, counterAdd)
		}
		processed++
	}
	...
	for {
		...
		case event, ok := <-watcher.Events:
			...
			if event.Op&(fsnotify.Write|fsnotify.Create) != 0 {
				...
				fmt.Fprintf(os.Stderr, "\033[90m[PRUNE] ?°ë“œ ?œëƒ…???œê±° ?„ë£Œ: %s\033[0m\n", filepath.Base(event.Name))
				queueUpdate()
			}
		case <-triggerChan:
			queueUpdate()
...
func gitSnapshot(brainRoot string) {
	...
	if _, err := os.Stat(gitDir); os.IsNotExist(err) {
		...
		if err := cmd.Run(); err != nil {
			fmt.Printf("[GIT] ? ï¸ init failed: %v\n", err)
			return
		}
	...
	// ?€?€ git diff ì§„í™”?ì •: ?´ëŸ° ?œê°?Œì´ë©??ë™ rollback ?€?€
	...
	if err == nil {
		...
		if deletions > insertions*2 && deletions > 5 {
			// ?? œê°€ ?½ì…??2ë°??´ìƒ?´ê³  5ê±?ì´ˆê³¼?´ë©´ ?´í™”ë¡??ì •
			fmt.Printf("[GIT] ? ï¸ ?´í™” ê°ì? (?? œ %d > ?½ì… %dÃ—2) ???ë™ rollback\n", deletions, insertions)
			revertCmd := exec.Command("git", "revert", "HEAD", "--no-edit")
			revertCmd.Dir = brainRoot
			if err := revertCmd.Run(); err != nil {
				fmt.Printf("[GIT] ??rollback ?¤íŒ¨: %v\n", err)
			} else {
				fmt.Println("[GIT] ???´í™” commit??revert ?˜ì—ˆ?µë‹ˆ??)
			}
		} else {
			fmt.Printf("[GIT] ??ì§„í™” ?ì • ?µê³¼ (ins:%d, del:%d)\n", insertions, deletions)
		}
	}
}
...
func runIdleLoop(brainRoot string) {
	...
	for {
		...
		// 0. Transcript Digestion ???„ì‚¬ ?Œì¼?ì„œ êµì • ??ì¶”ì¶œ ??neuronize
		apiKey := os.Getenv("GROQ_API_KEY")
		if apiKey != "" {
			pendingTurns := digestTranscripts(brainRoot)
			if pendingTurns >= 10 {
				fmt.Printf("[IDLE] ?§¬ ?„ì‚¬ ì²?¬ %d???„ì  ??Auto-Neuronize ?¤í–‰...\n", pendingTurns)
		...
		if apiKey != "" {
			...
			fmt.Println("[IDLE] ? ï¸  GROQ_API_KEY not set ??skipping evolve")
		}
		...
		if _, err := os.Stat(nasTarget); err == nil {
			...
			fmt.Println("[IDLE] ? ï¸  NAS Z: not available ??skipping sync")
		}
...
// digestTranscripts??_transcripts/ ?Œì¼?ì„œ êµì •/?ëŸ¬ ?´ì„ ì¶”ì¶œ?˜ì—¬
// corrections_history.jsonl??ì¶”ê??œë‹¤. cursor.json?¼ë¡œ ?„ì¹˜ ì¶”ì .
// ë°˜í™˜ê°? ?´ë²ˆ??ì¶”ì¶œ??êµì • ????
func digestTranscripts(brainRoot string) int {
	...
	correctionKeywords := []string{
		"?„ë‹ˆ", "?„ëƒ", "?˜ëª»", "?¤ì‹œ", "??", "?œë˜", "?ˆë¼", "?ˆë¨",
		"?ëŸ¬", "?¤ë¥˜", "?¤íŒ¨", "ë©ˆì¶¤", "404", "500",
		"ê¸ˆì?", "?˜ì?ë§?, "ë°˜ë“œ??, "??ƒ", "?ˆë?",
	}
	...
	for _, line := range lines {
		...
		// [HH:MM:SS PD] ?¨í„´?´ë©´ ?¬ìš©??ë°œí™”
		if !strings.Contains(line, " PD]") && !strings.Contains(line, "êµì •") {
			continue
		}
	...
	// êµì • ?´ì„ corrections_history.jsonl??ì¶”ê?
	if len(corrections) > 0 {
		...
		fmt.Printf("[DIGEST] ?“ ?„ì‚¬?ì„œ %dê±?êµì • ??ì¶”ì¶œ\n", len(corrections))
	}

	return len(corrections)
}

// writeHeartbeat??idle engine ?íƒœë¥?_heartbeat.json??ê¸°ë¡?˜ê³ ,
// ?´ëŸ° ??°œ ê°ì? ??git snapshot ??GEMINI.md???µí•© ì§€?œë? ì£¼ì…?œë‹¤.
func writeHeartbeat(brainRoot string, result SubsumptionResult) {
	...
	// ?´ëŸ° ??°œ ê°ì?: 20ê°??´ìƒ ì¦ê? ???µí•© ì§€??ì£¼ì…
	growth := result.TotalNeurons - prevNeurons
	if prevNeurons > 0 && growth >= 20 {
		fmt.Printf("[HEARTBEAT] ?”¥ ?´ëŸ° ??°œ ê°ì?: %d??d (+%d) ???µí•© ì§€??ì£¼ì…\n",
			prevNeurons, result.TotalNeurons, growth)

		// 1. git snapshot ? í–‰ (ë¡¤ë°± ë³´ì¥)
		fmt.Println("[HEARTBEAT] ?“¸ ?µí•© ??git snapshot...")
		gitSnapshot(brainRoot)

		// 2. GEMINI.md???µí•© ì§€??ì£¼ì…
		directive := fmt.Sprintf(
			"\n\n> [!IMPORTANT]\n> **[HEARTBEAT %s] ?´ëŸ° ??°œ ê°ì?: %d??d (+%d)**\n"+
				"> ? ì‚¬ ?´ëŸ° ?µí•©???„ìš”?©ë‹ˆ?? `neuronfs --dedup` ?¤í–‰ ?ëŠ” ?˜ë™?¼ë¡œ ? ì‚¬ ?´ë”ë¥?ë³‘í•©?˜ì„¸??\n"+
				"> git snapshot??? í–‰?˜ì—ˆ?¼ë?ë¡?ë¡¤ë°± ê°€?¥í•©?ˆë‹¤.\n",
			time.Now().Format("15:04"),
			prevNeurons, result.TotalNeurons, growth)

		// brainstem???µí•© ì§€???´ëŸ° ?ì„± (?„ì‹œ)
		consolidateDir := filepath.Join(brainRoot, "brainstem", "?´ëŸ°?µí•©_?„ìš”")
		...
		// writeAllTiersë¡?GEMINI.md ì¦‰ì‹œ ê°±ì‹ 
		writeAllTiers(brainRoot)

		fmt.Printf("[HEARTBEAT] ???µí•© ì§€??ì£¼ì… ?„ë£Œ ??brainstem/?´ëŸ°?µí•©_?„ìš” ?ì„±\n")
	}
}

// ?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”??
// DEDUP: ì¤‘ë³µ ?´ëŸ° ?´ë” ë³‘í•© (ì¹´ìš´???©ì‚°)
// ?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”?â”??
...
func deduplicateNeurons(brainRoot string) {
	...
	for _, region := range brain.Regions {
		if region.Name == "brainstem" {
			continue // brainstem?€ ?½ê¸° ?„ìš© ??ê±´ë“œë¦¬ì? ?ŠìŒ
		}
		for _, n := range region.Neurons {
			if n.IsDormant {
				continue
	...
	// Compare all pairs (O(nÂ²) ??200 ?´ëŸ°?´ë©´ ~20,000 ë¹„êµ, ë¬´ì‹œ???˜ì?)
	...
	for i := 0; i < len(allRefs); i++ {
		...
		for j := i + 1; j < len(allRefs); j++ {
			...
			fmt.Printf("[DEDUP] ?? ë³‘í•© (sim=%.2f): %s/%s (%d) ??%s/%s (%d) ??%d\n",
				sim,
			...
			// victim??dopamine/memory ?œê·¸?ì„ ?ì¡´?ë¡œ ?´ë™
	...
	if mergeCount > 0 {
		fmt.Printf("[DEDUP] ??%d ê±?ì¤‘ë³µ ?´ëŸ° ë³‘í•© ?„ë£Œ (ì¹´ìš´???©ì‚°+ë³´ë„ˆ??\n", mergeCount)
		writeAllTiers(brainRoot)
	} else {
		fmt.Println("[DEDUP] ??ì¤‘ë³µ ?´ëŸ° ?†ìŒ")
	}
}
...
func startAPI(brainRoot string, port int) {
	...
	// GET /api/integrity?region=cortex ??Merkle Hash Chain ë¬´ê²°??ê²€ì¦?
	// ?„ì²´: GET /api/integrity (7ê°??ì—­ ëª¨ë‘ ê²€ì¦?
	// ?¨ì¼: GET /api/integrity?region=cortex
	...
	// POST /api/community ???¸ë? ì»¤ë??ˆí‹° ?¸ë Œ?œë? ?´ëŸ°?¼ë¡œ ?˜ì§‘
	...
	mux.HandleFunc("/api/report", withCORS(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{
			...
			"ack": "?ˆë¡œ??ë³´ê³ ê°€ ?•ì¸?˜ì—ˆ?µë‹ˆ?? ?¬ìš©?ì˜ ?”ì²­ ì²˜ë¦¬ ???”ë¡œ?…í•©?ˆë‹¤.",
		})
	}))
