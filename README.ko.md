...
| 🔮 | [미션 크리티컬 확장의 원리](#미션-크리티컬-확장의-원리) | 어떻게 구조적 제약이 똑똑한 모델을 이기는가 |
...
#### Claude Desktop MCP 연동 예시

`claude_desktop_config.json`에 아래 설정을 추가하세요:
...
| 항목 | 현황 | 대응 |
|------|------|------|
| AI 강제력 | AI가 GEMINI.md를 100% 따르는 건 보장 불가 | 하네스가 위반 감지 → 교정 루프. **실측 94.9%** (brainstem 위반 18회 / 전체 353회 fire)* <br><sub>* 353회 fire: 에이전트가 단독 판단 후 실제 행동을 실행(Fire)한 개별 독립 세션. 즉, 353회의 자율 행동 중 시스템 최상위 계층 무시가 단 18회에 불과함.</sub> |
...
**Q: "아무나 .contra나 bomb 파일을 쓰면 시스템이 파괴되는 것 아니야?"**

맞다. NeuronFS의 보안은 전적으로 **OS 파일시스템 권한**과 **Git 무결성 검증**에 의존한다. 만약 누군가가 당신의 디렉토리 쓰기 권한을 탈취했다면, 프롬프트 인젝션 방어는 이미 무의미하다. 어설픈 블랙박스 벡터 DB의 추상적 보호보다, 전통적인 chmod 권한 통제나 Git 서명이 가장 투명하고 익숙한 실질적 방어막이라고 우리는 믿는다.
...
**실전 발동 기록:** bomb은 설계만 된 게 아니라 실제로 발동된 적 있다. 2026-03-29, cortex와 hippocampus에서 각각 1회. PD가 해제하여 정상 복구.

**해제 방법:** `rm brain_v4/cortex/.../bomb.neuron` — 파일 삭제가 전부다. CLI 명령 없다. "Everything is folders"이니까 파일 삭제 = 해제다. 의도적으로 단순하게 만들었다 — 비상 상황에서 복잡한 해제 절차는 실용성을 떨어뜨린다. 해제 후 다음 `--emit`에서 자동 복구.

**Q: "bomb이 발동되면 AI가 조용히 멈추는 거야?"**

아니다. **물리적으로 멈춘다.** bomb.neuron이 감지되면 `triggerPhysicalHook()`이 실행되어 USB 빨간 사이렌이 실제로 돌아간다. 에이전트가 멈추고, PD가 육안으로 확인하고, 원인 파악 후 `rm bomb.neuron`으로 해제한다.

이건 은유가 아니다 — **문자 그대로의 서킷 브레이커**다. 토큰 낭비를 막기 위한 하드 스탑이며, 소프트웨어 알림이 아니라 물리 경보다.

<p align="center">
  <img src="docs/bomb_alert.png" alt="bomb.neuron 발동 시 물리 경보 — 전체화면 빨간 플래시 + USB 사이렌" width="700" />
  <br/>
  <sub>bomb.neuron 감지 → 전체화면 빨간 플래시 + USB 사이렌 + 텔레그램 알림. 문자 그대로의 비상 정지.</sub>
</p>

```go
// physical_hooks.go — bomb 감지 시 OS 물리 인터럽트 발동
