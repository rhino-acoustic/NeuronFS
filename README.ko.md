...
| | NeuronFS | .cursorrules | Mem0 | RAG / 벡터 DB |
|---|---|---|---|---|
| **설치** | `mkdir` | 파일 생성 | pip + DB 서버 | pip + 임베딩 모델 + DB |
| **런타임 비용** | **$0** | $0 | $70+/월 | $50+/월 |
| **속도** | `ls` = ~1ms (syscall) | 파일 읽기 | 50-500ms (임베딩 + 검색) | 50-500ms |
| **종속성** | **없음** | IDE 종속 | Python, DB, API 키 | Python, DB, 모델 |
| **모델 종속** | **없음** — 어떤 AI든 뉴런을 읽음 | IDE 종속 | 임베딩 모델 | 임베딩 모델 |
| **멀티 에이전트** | NAS 공유 뇌 | 단일 프로젝트 | API 호출 | 복잡한 IPC |
| **상태 확인** | `tree brain/` | `cat` 파일 | API 쿼리 | API 쿼리 |
| **규칙 즉시 변경** | `touch` / `rm` — 즉시 | 파일 편집, 재시작 | DB 업데이트 | 재인덱싱 |
| **버전 관리** | `git diff` — 네이티브 | git 가능 | 비다윈적 | 비다윈적 |
| **오프라인 동작** | ✅ | ✅ | ❌ | ❌ |
| **시맨틱 검색** | ❌ 경로 기반만 | ❌ | ✅ | ✅ |
...
"교정 받자, 실행 안 해." 36번 교정받았다. 가장 높은 카운터. 36번의 교정이 뉴런 하나로 확정된다.
