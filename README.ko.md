...
하나의 설계 결정이 전체 시스템을 생성한다:

```
공리: "폴더가 뉴런이다."
  → 파일 경로 = 자연어 규칙
  → 파일명 = 활성화 횟수 (5.neuron = 5회 발화)
  → 폴더 접두어 = 거버넌스 유형 (禁=NEVER, 必=ALWAYS, 推=WHEN)
  → 깊이 = 구체성
  → OS 메타데이터 = 임베딩
  → mkdir = 학습
  → rm = 망각
...
이 공리 없이는 Merkle 체인, RBAC, 코사인 유사도, 서킷브레이커를 폴더에 결합할 이유가 없다. **NeuronFS를 NeuronFS로 만드는 건 알고리즘이 아니라 공리다.**
...
| 10 | **의존성** | IDE 종속 | Python+Redis+DB | Python+GPU+API | **런타임 제로 (단일 바이너리)** |
| 11 | **3-Tier 거버넌스** | ❌ | ❌ | ❌ | **✅ ALWAYS/WHEN/NEVER 자동 분류** |
| 12 | **OOM 보호** | ❌ | ❌ | ❌ | **✅ 컨텍스트 초과 시 자동 축소** |
...
| 테스트 | 항목 | 결과 | 업계 기준 |
|--------|------|------|----------|
| **BM-1** | 규칙 충실도 (AgentIF CSR) | **100%** (5/5) | IFEval SOTA: 95% |
| **BM-2** | 스캔 성능 (5K 뉴런) | **2.5s** | Mem0: 125ms (RAM) |
| **BM-3** | 유사도 정확도 | **P=1.0** F1=0.74 | 벡터DB: P≈0.85 |
| **BM-4** | 라이프사이클 (禁 보호) | **30/30 100%** | 업계 유일 |
| **BM-5** | 적대적 QA (LOCOMO) | **5/5 거부** | SQuAD 2.0 스타일 |
| **BM-6** | 프로덕션 레이턴시 | **p50=202ms p95=268ms** | Mem0 p50: 75ms |
| **BM-7** | 멀티홉 플래닝 (MCPBench) | **grow→fire→dedup→emit ✅** | 도구 체이닝 |
...
| 시스템 | 설명 | 왜 독자적인가 |
|--------|------|-------------|
| **Folder=Neuron** | `mkdir` = 뉴런 생성, 경로 = 자연어 규칙 | OS 폴더를 인지 단위로 쓴 시스템 없음 |
| **vorq 룬** | 16 runes (한자 12 + 조어 4) | AI 행동 제어용 인공 마이크로언어 |
| **3-Tier emit** | 禁→NEVER, 必→ALWAYS, 推→WHEN 자동 변환 | 규칙을 LLM에 "설치"하는 파이프라인 |
| **파일명=카운터** | `5.neuron` = 5회 활성화. DB 없음 | 메타데이터가 파일명 자체 |
| **bomb 서킷브레이커** | 3회 실패 → 인지 영역 전체 차단 | 인지 수준의 서킷브레이커 |
| **Hebbian 파일 스코어** | (활성화 × 1.5) + 무게 | 파일시스템의 시냅스 가중치 검색 |
| **emit → 모든 IDE** | 1개 뇌 → 4개 AI 동시 배포 | 단일 거버넌스가 모든 AI 지배 |
| **OOM 보호** | 토큰 초과 시 자동 축소 | 자기 컨텍스트 오버플로 방지 — 업계 유일 |

> **핵심: "폴더가 뉴런이다"라는 공리에서 나머지가 전부 파생된다.** 이 공리 없이는 기존 기술이 결합할 이유가 없다.
...
> *63 Go 소스파일, 297 함수, 190 테스트, ~22,000 줄. 단일 바이너리. 런타임 의존성 제로.*
